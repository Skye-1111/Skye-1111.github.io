...
class LidarSystem {
    ...
    handleRemoteControl(data) {
        if (data.clientId === this.clientId) return; // 忽略自己的指令
        
        this.log(`接收到远程控制指令 (${data.clientId}): ${data.data.command}`, 'info');
        
        // 执行远程控制指令
        this.executeRemoteCommand(data.data.command, data.data.parameters);
    }
    
    /**
     * 处理指令转发状态
     */
    handleCommandForwarded(data) {
    ...
    handleErrorResponse(data) {
        this.log(`设备错误: ${JSON.stringify(data.data)}`, 'error');
    }
    
    /**
     * 执行远程控制指令
     */
    executeRemoteCommand(command, parameters) {
        try {
            switch (command) {
                case 'connect_lidar':
                    this.connectLidar();
                ...
                case 'connect_stp23l':
                    this.stp23lSensor.connect();
                    break;
                    
                case 'disconnect_stp23l':
                    this.stp23lSensor.disconnect();
                    break;
                    
                case 'start_height_measure':
                    this.stp23lSensor.startMeasurement();
                    break;
                    
                case 'stop_height_measure':
                    this.stp23lSensor.stopMeasurement();
                    break;
                ...
                default:
                    this.log(`未知远程控制指令: ${command}`, 'warning');
            }
        } catch (error) {
            this.log(`执行远程控制指令失败: ${error.message}`, 'error');
        }
    }
     ...
     * 发送远程控制指令
     */
    sendRemoteControlCommand(command, parameters = null) {
        if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            this.log('WebSocket未连接，无法发送远程控制指令', 'error');
            return false;
        }
        ...
        this.log(`已发送远程控制指令: ${command}`, 'info');
        return true;
    }
